#!/usr/bin/env python3
"""
`make_catalog.py`: prepara LA VERSIONE COMPILABILE DEL SITO NEI PESI.

Il sito vero e' andato avanti (una landing semplice, una pagina per
macchina). Il modello invece conserva LA VERSIONE PRECEDENTE: "after the
bubble", il catalogo-frameset del 1996 — congelato in `www/oldsite/`
(sorgente storica: non si aggiorna col sito, si tocca solo per ripararla)
e servito per intero dal GGUF. Un pezzo da museo, custodito nei pesi.

Assembla `www/afterthebubble/` — cartella DERIVATA, non si edita a mano —
da `oldsite/` piu' le pagine che esistono solo dentro il modello
(`pages/`). Poi:
...
manifesto, il CSS, il JS — dentro il vocabolario di un GGUF. `sw.js` lo
serve nell'iframe di `index.html`: ogni frame, ogni foglio di stile, ogni
fetch del manifesto e' una GET diversa, e ogni GET e' due token di ollama.

Tre trasformazioni, tutte conseguenze di cosa puo' essere un token, non
scelte estetiche:

  - I BINARI DIVENTANO data: URI. Un token e' una stringa UTF-8: un PNG non
    ci passa (`compile_site.py` lo rifiuta, giustamente). Ma un PNG in
    base64 SI'. Quindi il logo finisce dentro il banner, `bg.png` dentro il
    foglio di stile e `construction.gif` dentro il JS, come data: URI. Il
    tassello di sfondo vive nei pesi, in base64, dentro il token del CSS.
    Non e' un trucco per aggirare il limite: e' il limite preso sul serio.

  - I LINK DELLE DEMO ESCONO DALLO SPECCHIO. Dentro il modello il catalogo
    sta sotto /www/neural/, ma le macchine vere (snake, wolf, ...) sono
    pagine normali servite da nginx. catalog.js scrive `../${links.demo}`,
    quindi qui il manifesto derivato porta `../../snake/index.html`:
    risalendo da /www/neural/site/ si torna alla radice vera. Le demo del
    museo restano vive.

  - LO SPECCHIO NON CONTIENE SE' STESSO. La demo di static-site-hosting e'
    `www/index.html`, cioe' la pagina che ospita questo specchio: lasciarla
    li' vorrebbe dire un iframe dentro se' stesso, all'infinito. Nel
    manifesto derivato quella demo punta a `about.html` — servita dal
    modello, che e' esattamente cio' che la macchina fa.
"""
...
SRC = os.path.join(HERE, "oldsite")
OUT = os.path.join(HERE, "afterthebubble")

# pagine del sito storico che entrano nello specchio (path servito -> sorgente)
SITE = {
...
INLINE = [
    ("site/banner.html", "site/assets/logo.png", 'src="assets/logo.png"',
     "image/png", 'src="data:{mime};base64,{b64}"'),
...
def inline_assets(text, rel):
    ...
    se il riferimento non c'e' piu': vorrebbe dire che il sito storico e'
    stato toccato e questo script sta zitto mentre lo specchio si rompe."""
    for src, asset, needle, mime, tmpl in INLINE:
        ...
        if needle not in text:
            sys.exit(
                ...
                f"  oldsite/ e' cambiato: aggiorna INLINE in make_catalog.py,\n"
